#!/usr/bin/env python
"""
Merge random-control LoRA adapter into base model.

Creates a standalone merged model for the synergy pipeline.
"""
...
def main():
    ...
    if os.path.exists(os.path.join(merged_dir, "config.json")):
        ...
        logger.info(
            f"Merged model: {spec.num_layers} layers, {spec.num_heads} heads/layer, "
    ...
    logger.info(
        f"Base model: {spec_before.num_layers} layers, "
        f"{spec_before.num_heads} heads/layer"
    )
    ...
    logger.info(
        f"Merged model: {spec_after.num_layers} layers, "
        f"{spec_after.num_heads} heads/layer"
    )

    assert spec_before.num_layers == spec_after.num_layers, \
        "Layer count changed after merge!"
    assert spec_before.num_heads == spec_after.num_heads, \
        "Head count changed after merge!"
    assert spec_before.head_dim == spec_after.head_dim, \
        "Head dim changed after merge!"
    ...
    logger.info("Random control merge complete.")
